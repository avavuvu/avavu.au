<script lang="ts">
	...
	const { children, heading, depth, colStart, colSpan = 2, scrollTo = "bottom" } = $props<{
		children: Snippet,
		heading: Snippet,
		depth: number
		colStart: number,
...
<div 
	class:needs-background={colStart === 1}
	style:opacity="{isLarge.current ? "100" : opacity}%"
	style:grid-column-start={colStart} 
	style:grid-column-end="{colStart + colSpan}" 
	class="content transition-opacity duration-1000  overflow-y-scroll min-h-0 md:max-h-full"  
	bind:this={block}
	in:fade>
	<Heading {level}> 
		{@render heading()}
